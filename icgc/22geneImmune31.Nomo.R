...
options(datadist="dd")
#生成函数
f <- cph(Surv(futime, fustat) ~ riskScore+Age+Gender+Stage, x=T, y=T, surv=T, data=rt, time.inc=1)
surv <- Survival(f)
#建立nomogram
...
#calibration curve
time=3    #预测年限
f <- cph(Surv(futime, fustat) ~ riskScore+Age+Gender+Stage, x=T, y=T, surv=T, data=rt, time.inc=time)
